"""VFM for CIFAR-10 4x super-resolution: a Gaussian adapter trained jointly with the flow map.

The flow map is the py-meanflow CIFAR U-Net, initialised from net_ema1 and left trainable,
so both networks are updated from step 0. The operator downsamples a 32x32 image to 8x8 by
bicubic interpolation with antialiasing, and measurement noise of 0.05 is added to every
observed value. Unlike inpainting, every pixel of y is observed, and the residual floor is
0.693. The adapter takes the observation upsampled back to 32x32, which lets it keep the
same architecture as the inpainting version.

The loss is a port of the authors' ImageNet AdapterLoss and has three terms. The data term
is the residual on the observation, decoded through an EMA copy of the flow map so that it
trains only the adapter. The KL term keeps the adapter close to N(0, I). The flow term keeps
the flow map a valid flow map, and is the mean of the flow matching and MeanFlow losses,
taken on interpolants whose noise endpoint is the adapter's z with probability ALPHA and
fresh noise otherwise. The three terms are summed and then rescaled by 1/(loss + 0.01).

Both networks use AdamW at 1e-4 with no weight decay. The paper uses alpha = 0.5 for images.

Evaluation reports the residual on the observation, the diversity of the samples measured by
LPIPS over whole images, and how far the flow map has drifted from its pretrained state,
measured by decoding a fixed batch of latents at every probe.
"""
import os, sys, math, time, copy
import torch, torch.nn as nn, torch.nn.functional as F

sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "common"))  # repo layout
from flow_map import COMMON
device = "cuda"
D = 3 * 32 * 32
NAME = os.environ.get("NAME", "vfmsr01")
...
RES_MAX = float(os.environ.get("RES_MAX", "4"))  # watchdog (SR scale): stop if res_raw > RES_MAX for 3 probes after step 3000
SIG = 0.05
def op(x):
    return F.interpolate(x, size=8, mode="bicubic", antialias=True)
def y_up(y):
    return F.interpolate(y, size=32, mode="bicubic", align_corners=False)
N_OBS = 8 * 8 * 3
RES_PERFECT = SIG * math.sqrt(N_OBS)  # 0.693
...
# eval targets
_tg = torch.load(f"{COMMON}/rf_targets.pt", map_location=device, weights_only=True)
X_EVAL, Y_EVAL = _tg["X"].to(device), _tg["Y"].to(device)  # (8,3,32,32), (8,3,8,8)
_lp = None
def lpips_pairs(a, b):
    global _lp
    if _lp is None:
...
# no hole here, so diversity and truth-LPIPS are on whole images
...
def probe(step):
    ...
    for k in range(Y_EVAL.shape[0]):
        ...
        for mod, acc in ((net_ema, res_e), (net, res_r)):
            ...
            acc.append((((op(xh) - yk)) ** 2).flatten(1).sum(1).sqrt().mean().item())
            if mod is net:  # metrics on the raw map, the one we would deploy, since the EMA lags the adapter
                divs.append(lpips_pairs(xh[:32], xh[32:]).mean().item())
                lpt.append(lpips_pairs(xh, xk).mean().item())  # full-image LPIPS vs truth
...
def loss_on(x):
    ...
    y = op(x) + SIG * torch.randn(x.shape[0], 3, 8, 8, device=x.device)
    mu, lv = adapter(y_up(y))
    z = mu + (0.5 * lv).exp() * torch.randn_like(mu)
    # 1. data consistency through EMA theta^- (grads reach adapter only)
    ones = torch.ones(B, device=device)
    xhat = z - u_of(net_ema, z, ones, ones)
    L_data = 200.0 * ((op(xhat) - y) ** 2).mean(dim=(1, 2, 3))
...
for i in range(1, STEPS + 1):
    ...
    if i % 250 == 0 or i == STEPS:
        ...
        print(f"{NAME} {i}: res_ema={re_:.3f} res_raw={rr_:.3f} (floor {RES_PERFECT:.3f}) "
              f"div={dv_:.3f} lpips_true={lpt_:.3f} znorm={zn_:.1f} prior_drift={drift:.2f} lr={lr_at(i):.1e} "
              f"[data {L_data.mean():.3f} kl {L_kl.mean():.4f} fm {L_fm.mean():.3f} mf {L_mf.mean():.3f}] "
              f"({(time.time()-t0)/60:.1f}m){tag}", flush=True)
        if WB is not None:
            WB.log({"eval/res_ema": re_, "eval/res_raw": rr_, "eval/div": dv_, "eval/lpips_true": lpt_,
